...
class MaterialPurchaseRequisition(models.Model):
    ...
    employee_id = fields.Many2one(
        'res.users',
        string='User',
        default=lambda self: self.env['res.users'].search([('user_id', '=', self.env.uid)], limit=1),
        required=True,
        copy=True,
    )
    ...
    def requisition_reject(self):
        for rec in self:
            rec.state = 'reject'
            rec.reject_employee_id = self.env['res.users'].search([('user_id', '=', self.env.uid)], limit=1)
    ...
    def manager_approve(self):
        for rec in self:
            ...
            rec.approve_manager_id = self.env['res.users'].search([('user_id', '=', self.env.uid)], limit=1)
    ...
    def user_approve(self):
        for rec in self:
            ...
            rec.approve_employee_id = self.env['res.users'].search([('user_id', '=', self.env.uid)], limit=1)
            rec.state = 'approve'
